...
//store list for actors to process each frame
this.actors = [];
...
function Coin(pos){
  ...
  this.woble = Math.random() * Math.PI * 2;
}
...
Level.prototype.obstacleAt = function (pos, size) {
  ...
  ///consider sides, top and bottom of level as walls
  if(xStart < 0 || xEnd > this.width || yStart < 0 || yEnd > this.height)
    return "wall";

/// check to see if objects are in the way and return said object
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  var obstacle= level.obstacleAt(newPos, this.size);
  var failAttempt = new Vector(2, 5);
  ///floor is also an obstacle
  // can only jump if player is touching an obstacle
  if (obstacle) {
